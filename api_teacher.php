...
if (isset($_SESSION['logged']) && $_SESSION['logged'] == true) {
    ...
    // Hardcoded values
    $id = 9;
    $teachername = 'Mr.Ketan Jadhav';
    $education = 'M.Tech';
    $branch = 'Computer Science And Engineering';
    $designation = 'Assistant Professor';
    $password = md5('pass123'); // Use any default password you'd like to hash

    // Skip photo upload or assign a default
    $photo_name = '"C:\xampp\htdocs\qr-code-attendance-system-master\img\default.jpg.jpg"'; // Or you can leave blank if photo not required

    $sql = "INSERT INTO `teachers`(`name`, `education`, `designation`, `branch`, `password`, `photo`)
            VALUES ('$teachername','$education','$designation','$branch','$password','$photo_name')";
    $result = mysqli_query($conn, $sql);

    if ($result) {
        $_SESSION['msg'] = '<div class="alert alert-success mb-2" role="alert">Teacher Added.</div>';
    } else {
        $_SESSION['msg'] = '<div class="alert alert-danger mb-2" role="alert">Something went wrong!</div>';
    }
